...
const buildScreenReaderWeather = (weather, location) => {
    ...
    return `${weather.current.weather[0].description} and ${_PV(weather.current.temp)}°${tmpUnit} in ${loc}`;
}
...
const createCurrentConditionsDisplay = (weather, units) => {
    ...
    const icon = createMainImgDiv(weather.current.weather[0].icon, weather.current.weather[0].description);
